...
use {
    ...
    }, solana_account_info::{next_account_info, AccountInfo}, solana_cpi::{invoke, invoke_signed}, solana_msg::msg, solana_program_error::{ProgramError, ProgramResult}, solana_program_pack::Pack, solana_pubkey::Pubkey, solana_rent::Rent, solana_system_interface::instruction::{allocate, assign}, solana_sysvar::Sysvar, solana_zk_sdk::encryption::pod::elgamal::PodElGamalPubkey, spl_token_2022::{
        extension::PodStateWithExtensions,
        instruction::initialize_mint2,
        onchain::{extract_multisig_accounts, invoke_transfer_checked},
        pod::{PodAccount, PodMint}, state::Mint,
    }
};
...
/// Processes [`CreateMint`](enum.TokenWrapInstruction.html) instruction.
pub fn process_create_confidential_mint(
    program_id: &Pubkey,
    accounts: &[AccountInfo],
    idempotent: bool,
    ...
    let auditor_pubkey = if auditor_pubkey.eq(&[0u8; 32]) {
        None
    } else {
        let ap = PodElGamalPubkey::from(auditor_pubkey);
        Some(ap)
    };

    let approve_authority = if approve_authority.eq(&[0u8; 32]) {
        None
    } else {
        Some(Pubkey::new_from_array(approve_authority))
    };
...
pub fn process_instruction(
    ...
    match TokenWrapInstruction::unpack(input)? {
        ...
        TokenWrapInstruction::CreateConfidentialMint { 
            ...
            msg!("Instruction: CreateConfidentialMint");
            process_create_confidential_mint(program_id, accounts, idempotent, auto_approve_new_accounts, approve_authority, auditor_pubkey)
        }
    }
}
